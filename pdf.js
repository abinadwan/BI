...
(function () {
  ...
  // Use local fonts through App.ensurePdfFonts; keep legacy helper removed
  ...
  function renderPdfPreview(options) {
    ...
    const title = locale === 'ar' ? ????? ??????? ????????? �  : Material Cost Estimator � ;
  ...
  async function exportPDF(options) {
    try {
      ...
      await App.ensurePdfFonts(doc, rtl ? 'ar' : 'en');
      ...
      const fontName = rtl ? 'AppFont' : ((doc.getFont && doc.getFont().fontName) ? doc.getFont().fontName : 'helvetica');\n\n      doc.autoTable({ startY: opts.margin.top + 20,
        head,
        body,
        foot,
        styles: { font: 'AppFont', fontSize: 8, cellPadding: 2, halign: rtl ? 'right' : 'left' },
        ...
        didDrawPage: (data) => {
          ...
          if (opts.showFooter) {
            ...
            const footerText = (locale==='ar') ? ??????  ??  : Page  of ;
            doc.setFontSize(8);
            doc.text(footerText, pageWidth - opts.margin.right - 20, pageHeight - 6, { align: 'right' });
            const genText = (locale==='ar') ? ?? ??????? ?????? AdwanX �  : Generated by AdwanX � ;
